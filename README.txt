...
For questions, comments or remarks about the code please contact S. Michiels (stefan.michiels@gustaveroussy.fr) or S. Belhechmi (shayma.BEL-HECHMI@gustaveroussy.fr).

The code has been written using R. 

To reproduce the results presented in the article, it is sufficient to execute the files in the following "./functions" folder:
...
Of note, the analyses are performed using parallelization, which prevents us from fixing a seed for each replicate. Therefore, the results cannot be reproduced exactly, but only approximately.
The R code is licensed under the GNU General Public License: you may redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation, version 3 of the License.